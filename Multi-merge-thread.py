...
class teleoperation:
    ...
    def set_velocity_goal(self, v, base=1.1, max_gain=1.2, threshold=0.01):
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                if self.count == 2000:
                    print(f"Teleoperation running rate: {1/(now - last_time)}")
                    self.count = 0
                self.count += 1
...
class model(threading.Thread):
    ...
    def run(self):
        while self.running:
            ...
            if self.count == 400:
                print(f"Model running in rate: {1/elapsed}")
                self.count = 0
            self.count += 1
