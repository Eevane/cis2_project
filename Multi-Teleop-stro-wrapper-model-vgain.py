...
class teleoperation:
    ...
    def set_velocity_goal(self, v, base=1.12, max_gain=1.2, threshold=0.03):
        norm = numpy.linalg.norm(v)
        print(f"velocity norm {norm}")
        if norm < threshold:
            gain = max_gain
        else:
    ...
    def velocity_gain_per_axis(v, base_gains=(1.12, 1.12, 1.2), max_gains=(1.4, 1.4, 1.25), thresholds=(0.02, 0.02, 0.03)):
    ...
    def externalforce_prediction(self, component):
        ...
        if component == self.master1:
            ...
            slot_end = time.time()
            print(f"model inference time: {slot_end - slot_model}")
        ...
        # if component.name.startswith('PSM'):
        #     print(f"part 1 cost: {slot2 - slot1}")
        #     print(f"part 2 cost: {slot3 - slot2}")
        #     print(f"part 3 cost: {slot4 - slot3}")
        #     print(f"part 4 cost: {slot5 - slot4}")
        #     print(f"part 5 cost: {slot6 - slot5}")
    ...
    def run_following(self):               
        ...
        # print(f"MTML external force prediction costs: {time.time()-start1}")
        ...
        # print(f"MTMR external force prediction costs: {time.time()-start2}")
        ...
        # print(f"PSM external force prediction costs: {time.time()-start3}")
        ...
        # force input
        gamma = 1
        # print(f"master1_external_f: {master1_external_f}")
        # print(f"master2_external_f: {master2_external_f}")
        ...
        # print(f"puppet force goal:{force_goal}")
        # Move
        ...
        master1_measured_cv[0:3] /= 0.25
        # master2_measured_cv[0:3] /= self.velocity_scale
        master2_measured_cv[0:3] /= 0.25
...
class ARM:
    class LoadModel:
        def __init__(self, onnx_path, param_path):
            self.ort_session = onnxruntime.InferenceSession(onnx_path, providers=["GPUExecutionProvider"])
...
if __name__ == '__main__':
    ...
    path_root = "/home/xle6/dvrk_teleop_data/July_31/200Hz/checkpoints/len1/"
