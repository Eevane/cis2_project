...
# low pass force filter
def filter_force(sos, value, state):
    y, state = sosfilt(sos, [value], zi=state)
    return y[0], state
...
class teleoperation:
    ...
    def __init__(self, mtml1, mtml2, psm2, clutch, coag, alpha, beta, clutch_topic, run_period, align_mtm, operator_present_topic = ""):
        ...
        fs = int(1/self.run_period)
        fc = 30
        self.sos = butter(N=2, Wn=fc, fs=fs, btype='low', output='sos')
        self.m1_state = [sosfilt_zi(self.sos) * 0 for _ in range(3)]  # three dimension
        self.m2_state = [sosfilt_zi(self.sos) * 0 for _ in range(3)]  # three dimension
        self.p_state = [sosfilt_zi(self.sos) * 0 for _ in range(3)]  # three dimension
    ...
    def run_following(self):
        ...
        # apply force filter
        for i in range(3):
            m1_measured_cf_force[i], self.m1_state[i] = filter_force(self.sos, m1_measured_cf_force[i], self.m1_state[i])
            m2_measured_cf_force[i], self.m2_state[i] = filter_force(self.sos, m2_measured_cf_force[i], self.m2_state[i])
            puppet_measured_cf_force[i], self.p_state[i] = filter_force(self.sos, puppet_measured_cf_force[i], self.p_state[i])
