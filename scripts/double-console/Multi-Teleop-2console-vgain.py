...
class teleoperation:
    ...
    def __init__(self, clutch_topic, run_period, align_mtm, operator_present_topic="", alpha = 0.5, beta = 0.5, **kwargs):
        num = len(kwargs)
        ...
        assert 0.0 <= beta <= 1.0 , "Dominance factor should be at [0, 1]."
        self.alpha = alpha
        self.beta = beta
    ...
    def run_following(self):
        ...
        # force input
        gamma = 0.714
        force_r_goal = self.force_gain * (self.beta * master1_r_measured_cf + (1 - self.beta) * master2_r_measured_cf + gamma * puppet_r_measured_cf)
        ...
        master_r_total_translation = (self.alpha * master1_r_translation + (1 - self.alpha) * master2_r_translation) / 2.0
        ...
        raw_puppet_r_velocity_goal = (self.alpha * master1_r_measured_cv + (1 - self.alpha) * master2_r_measured_cv) / 2.0
        ...
        # average gripper lag
        ghost_r_lag = (master1_r_ghost_lag + master2_r_ghost_lag) / 2.0
        ...
        # set translation of mtm1
        master1_r_position = puppet_r_translation / max(self.alpha,1e-8) + master1_r_initial_trans
        # set translation of mtm2
        master2_r_position = puppet_r_translation / max(self.alpha,1e-8) + master2_r_initial_trans
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--beta', type = float, required = False, default= 0.5,
                         help = 'dominance factor of force channel, between 0 and 1')
    ...
    application = teleoperation(clutch, args.interval,
                                ...
                                MTML_novice=mtml1, MTML_expert=mtml2, MTMR_novice=mtmr1, MTMR_expert=mtmr2, PSML=psm1, PSMR=psm2)
