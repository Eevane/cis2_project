...
class teleoperation:
    ...
    def run_following(self):
        ...
        """  """
        # master1
        master1_l_external_f = self.externalforce_prediction(self.master1_l)   # (6,) numpy array
        master1_l_external_f[0:3] *= -1.0
        master1_l_external_f[3:6] *= 0   # turn off torque

        # master2
        master2_l_external_f = self.externalforce_prediction(self.master2_l)   # (6,) numpy array
        master2_l_external_f[0:3] *= -1.0
        master2_l_external_f[3:6] *= 0   # turn off torque

        # puppet
        puppet_l_external_f = self.externalforce_prediction(self.puppet_l)
        ...
        """  """
        # master1
        master1_r_external_f = self.externalforce_prediction(self.master1_r)   # (6,) numpy array
        master1_r_external_f[0:3] *= -1.0
        master1_r_external_f[3:6] *= 0   # turn off torque

        # master2
        master2_r_external_f = self.externalforce_prediction(self.master2_r)   # (6,) numpy array
        master2_r_external_f[0:3] *= -1.0
        master2_r_external_f[3:6] *= 0   # turn off torque

        # puppet
        puppet_r_external_f = self.externalforce_prediction(self.puppet_r)
