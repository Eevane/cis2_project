...
class teleoperation:
    ...
    def externalforce_prediction(self, component):
        ...
        measured_q, measured_dq, measured_torque = component.measured_js()
        q = measured_q[:6]
        dq = measured_dq[:6]
        ...
        elif component == self.master2_l:
            self.queue_MTML2_first3 = numpy.concatenate((self.queue_MTML2_first3, first_input), axis=1)
            self.queue_MTML2_last3 = numpy.concatenate((self.queue_MTML2_last3, last_input), axis = 1)
            self.queue_MTML2_first3 = self.queue_MTML2_first3[:, 1:, :]
            self.queue_MTML2_last3 = self.queue_MTML2_last3[:, 1:, :]

            # model predict
            first_ort_inputs = {component.firstmodel.ort_session.get_inputs()[0].name: self.queue_MTML2_first3.astype(numpy.float32)}
            first_ort_outs = component.firstmodel.ort_session.run(None, first_ort_inputs)
            last_ort_inputs = {component.lastmodel.ort_session.get_inputs()[0].name: self.queue_MTML2_last3.astype(numpy.float32)}
            last_ort_outs = component.lastmodel.ort_session.run(None, last_ort_inputs)

        elif component == self.master2_r:
            self.queue_MTMR2_first3 = numpy.concatenate((self.queue_MTMR2_first3, first_input), axis=1)
            self.queue_MTMR2_last3 = numpy.concatenate((self.queue_MTMR2_last3, last_input), axis = 1)
            self.queue_MTMR2_first3 = self.queue_MTMR2_first3[:, 1:, :]
            self.queue_MTMR2_last3 = self.queue_MTMR2_last3[:, 1:, :]

            # model predict
            first_ort_inputs = {component.firstmodel.ort_session.get_inputs()[0].name: self.queue_MTMR2_first3.astype(numpy.float32)}
            first_ort_outs = component.firstmodel.ort_session.run(None, first_ort_inputs)
            last_ort_inputs = {component.lastmodel.ort_session.get_inputs()[0].name: self.queue_MTMR2_last3.astype(numpy.float32)}
            last_ort_outs = component.lastmodel.ort_session.run(None, last_ort_inputs)

        elif component == self.puppet_l:
            self.queue_PSM2_first3 = numpy.concatenate((self.queue_PSM2_first3, first_input), axis=1)
            self.queue_PSM2_last3 = numpy.concatenate((self.queue_PSM2_last3, last_input), axis = 1)
            self.queue_PSM2_first3 = self.queue_PSM2_first3[:, 1:, :]
            self.queue_PSM2_last3 = self.queue_PSM2_last3[:, 1:, :]

            # model predict
            first_ort_inputs = {component.firstmodel.ort_session.get_inputs()[0].name: self.queue_PSM2_first3.astype(numpy.float32)}
            first_ort_outs = component.firstmodel.ort_session.run(None, first_ort_inputs)
            last_ort_inputs = {component.lastmodel.ort_session.get_inputs()[0].name: self.queue_PSM2_last3.astype(numpy.float32)}
            last_ort_outs = component.lastmodel.ort_session.run(None, last_ort_inputs)
...
class model(threading.Thread):
    def __init__(self, master1_l, master2_l, puppet1, master1_r, master2_r, puppet2, freq):
        super().__init__()
    ...
    def run(self):
        ...
        while self.running:
            ...
            master1_l_external_f = self.externalforce_prediction(self.master1_l)
            master2_l_external_f = self.externalforce_prediction(self.master2_l)
            puppet1_external_f = self.externalforce_prediction(self.puppet1)
            master1_r_external_f = self.externalforce_prediction(self.master1_r)
            master2_r_external_f = self.externalforce_prediction(self.master2_r)
            puppet2_external_f = self.externalforce_prediction(self.puppet2)
...
if __name__ == '__main__':
    ...
    model_thread = model(mtml1, mtml2, psm2, mtmr1, mtmr2, psm1, freq=200)  
    model_thread.start()
    application.run()
