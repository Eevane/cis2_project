...
"""Turn off force feedback at three wrist axes."""
...
class teleoperation:
    ...
    def run_following(self):
        ...
        m1_measured_jf[-3:] = 0   # turn off force from the last three axis
        ...
        m2_measured_jf[-3:] = 0   # turn off force from the last three axis
        ...
        # master3
        '''Measure force from joint space'''
        m3_measured_js = self.master3.measured_js()
        m3_measured_jf = m3_measured_js.effort() ??
        m3_measured_jf[-3:] = 0   # turn off force from the last three axis
        m3_body_jacobian = self.master3.body.jacobian()
        m3_body_jacobian_trans_inv = numpy.linalg.pinv(m3_body_jacobian.T)
        m3_measured_cf_force = m3_body_jacobian_trans_inv @ m3_measured_jf
        m3_measured_cf_force[0:3] = m3_measured_cf_force[0:3] * (-1)
        m3_measured_cf_force[3:6] = m3_measured_cf_force[3:6] * 0 * 2
        ...
        # master4
        '''Measure force from joint space'''
        m4_measured_js = self.master4.measured_js()
        m4_measured_jf = m4_measured_js.effort() ??
        m4_measured_jf[-3:] = 0   # turn off force from the last three axis
        m4_body_jacobian = self.master4.body.jacobian()
        m4_body_jacobian_trans_inv = numpy.linalg.pinv(m4_body_jacobian.T)
        m4_measured_cf_force = m4_body_jacobian_trans_inv @ m4_measured_jf
        m4_measured_cf_force[0:3] = m4_measured_cf_force[0:3] * (-1)
        m4_measured_cf_force[3:6] = m4_measured_cf_force[3:6] * 0 * 2
