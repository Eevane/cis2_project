...
""" Strong-connected structure, neural network deployer """
...
class teleoperation:
    ...
    def __init__(self, ral, mtm1, mtm2, puppet, clutch_topic, run_period, align_mtm, operator_present_topic = "", alpha = 0.5, beta = 0.5):
        ...
        self.m2_force = []
        self.puppet_force = []
        self.internal_torque_record = []
        self.total_torque_record = []
    ...
    def externalforce_prediction(self, component):
        ...
        # model predict
        first_ort_inputs = {component.firstmodel.ort_session.get_inputs()[0].name: first_input.astype(numpy.float32)}
        first_ort_outs = component.firstmodel.ort_session.run(None, first_ort_inputs)
        last_ort_inputs = {component.lastmodel.ort_session.get_inputs()[0].name: last_input.astype(numpy.float32)}
        ...
        self.internal_torque_record.append(internal_torque.reshape(-1).tolist())
        self.total_torque_record.append(total_torque)
    ...
    def run_following(self):               
        ...
        if self.a == 5000:
            ...
            numpy.savetxt('/home/pshao7/dvrk_python_devel/dataset/multi_internal_torque_0628.txt', self.internal_torque_record, fmt='%f', delimiter=' ', header='Joint1 Joint2 Joint3 Joint4 Joint5 Joint6', comments='')
            numpy.savetxt('/home/pshao7/dvrk_python_devel/dataset/multi_total_torque_0628.txt', self.total_torque_record, fmt='%f', delimiter=' ', header='Joint1 Joint2 Joint3 Joint4 Joint5 Joint6', comments='')
